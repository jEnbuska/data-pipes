...
export type StreamlessProvider<TOutput, TReturn = unknown> = () => Generator<
  TOutput,
  TReturn,
  undefined & void
>;
...
export type StreamlessResolvers<
  TInput,
  TAsync extends boolean = false,
  TDefault = undefined,
> = {
...
export type SyncStreamless<
  TInput = unknown,
  TDefault = undefined,
> = StreamlessFunctions<TInput, false, TDefault> & {
  isAsync: false;
...
export type AsyncStreamless<
  TInput = unknown,
  TDefault = undefined,
> = StreamlessFunctions<TInput, true, TDefault> & {
  isAsync: true;
};

type StreamlessFunctions<
  TInput = unknown,
  TAsync extends boolean = false,
  TDefault = undefined,
> = Readonly<
  StreamlessResolvers<TInput, TAsync, TDefault> & {
    /**
    ...
    ): Streamless<MaybeAwaited<TOutput, TAsync>, TAsync>;
    /**
    ...
    ): Streamless<MaybeAwaited<TInput[], TAsync>, TAsync, TInput[]>;
    /**
    ...
    flat<Depth extends number = 1>(
      depth?: Depth,
    ): Streamless<FlatArray<TInput[], Depth>, TAsync>;
    flatMap<TOutput>(
     ...
     *   .toArray() // [1,3];
     */
    filter(fn: (next: TInput) => any): Streamless<TInput, TAsync>;
    /**
    ...
    ): Streamless<TOutput, TAsync, TOutput>;
    /**
    ...
    ): Streamless<TInput, TAsync, TDefault>;
    /**
     ...
     *  .toArray() // [3]
     */
    skip(count: number): Streamless<TInput, TAsync>;
    /**
     ...
     *  .toArray() // [1]
     */
    skipLast(count: number): Streamless<TInput, TAsync>;
    /**
     ...
     *  .toArray() // [3,4]
     * */
    skipWhile(fn: (next: TInput) => boolean): Streamless<TInput, TAsync>;
    /**
     ...
     *  .toArray() // [1,2]
     */
    take(count: number): Streamless<TInput, TAsync>;
    /**
     ...
     *  .toArray() // [2,3]
     */
    takeLast(count: number): Streamless<TInput, TAsync>;
    /**
     ...
     *  .first() // 3
     */
    count(): Streamless<number, TAsync, number>;
    /**
     ...
     *  .toArray() // [1,2]
     */
    takeWhile(fn: (next: TInput) => boolean): Streamless<TInput, TAsync>;
    /**
     ...
     *  .first() // 1
     */
    min(selector: (next: TInput) => number): Streamless<TInput, TAsync>;
    /**
     * takes each item produced by the generator and maps it to a number using the callback.
     ...
     *  .first() // 4
     */
    max(selector: (next: TInput) => number): Streamless<TInput, TAsync>;
    /**
    ...
    defaultTo<TDefault = TInput>(
      getDefault: () => TDefault,
    ): Streamless<TInput | TDefault, TAsync, TDefault>;
    /**
     ...
     *  .toArray() // [3]
     */
    find(fn: (next: TInput) => boolean): Streamless<TInput, TAsync>;
    /**
     ...
     *  .first() // true
     */
    some(fn: (next: TInput) => boolean): Streamless<boolean, TAsync, false>;
    /**
     ...
     *  .first() // false
     */
    every(fn: (next: TInput) => boolean): Streamless<boolean, TAsync, true>;
    /**
     ...
     *  .toArray() // [3,2,1]
     */
    reverse(): Streamless<TInput, TAsync>;
    /**
