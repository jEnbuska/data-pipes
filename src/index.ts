import { asyncIterableAYielded } from "./create/asyncYielded.ts";
...
function fromCallable<TOutput>(
  callback: () => Iterable<TOutput, unknown | void, undefined & void>,
): void;
 ...
 * })
 *  .groupBy(user => user.id) satisfies Promise<Record<string, User>>
 *
 * @example
 * yielded([1,2,3])
 *  .map(n => n * 2)
 ...
 * yielded(1)
 *  .map(n => n * 2)
 *  .toArray() satisfies number | undefined // 2
 */
...
function isAsyncGeneratorFunction<TInput>(
  provider: unknown,
): provider is YieldedAsyncGenerator<TInput> {
  return (
    Boolean(provider) &&
