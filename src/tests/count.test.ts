...
describe("count", () => {
  ...
  test("from async generator", async () => {
    expect((await fromAsyncGenerator.count().first()) satisfies number).toBe(
      numbers.length,
    );
  });
  ...
  test("from promises", async () => {
    expect(
      (await fromPromises.resolve().count().first()) satisfies number,
    ).toBe(numbers.length);
  });
  ...
  test("from empty async", async () => {
    expect((await fromEmptyAsync.count().first()) satisfies number).toBe(0);
  });
});
